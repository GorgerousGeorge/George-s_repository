...
def card_number_generator(start: int, finish: int) -> str:
    """Генерирует 16-значные номера карт в рамках заданнного диапазона. При вызове генератора первым аргументом передаем
    начальное значение генерации диапазона, вторым аргументом передаем конечное значение диапазона (включительно)"""
    ...
    for card_number in range(start, finish + 1):
        ...
        return_number = (return_number[:4] + " " + return_number[4:8] + " " + return_number[8:12] + " " +
                         return_number[12:])
        yield return_number
